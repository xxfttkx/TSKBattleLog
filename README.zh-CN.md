...
![战斗日志窗口](screenshots/gui_战斗日志.png)

*战斗日志窗口：战斗时间线——回合分隔（含累计伤害）、按攻击者的技能分组（按属性着色）、Unison 事件——下半部分为每段伤害系数明细面板*
...
- Unison 伤害追踪
- 战斗日志窗口：战斗时间线（回合分隔含累计伤害、按攻击者的技能分组并按属性着色、Unison 事件），点击分组展开每段伤害系数明细——乱数 / Rush / 属性克制 / 暴击 / Down / 易伤倍率 / 被动
...
- **战斗日志**按钮：打开战斗日志窗口（按需快照，点刷新拉取最新；窗口大小/位置记忆）
...
| 键                  | 分类 | 说明                                                                                               |
|---------------------|------|----------------------------------------------------------------------------------------------------|
| `battle-log`        | 观察 | 打印战斗相关各种信息（伤害统计、多段分组、回合、战斗结算）；驱动战斗日志窗口，提供每段伤害系数明细 |
